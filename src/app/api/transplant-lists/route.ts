...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.request().input('pid', projectId).query(`
      SELECT tl.*,
        (SELECT id, fromLocation, toLocation, count
         FROM TransplantListItem WHERE transplantListId = tl.id
         FOR JSON PATH) AS itemsJson
      FROM TransplantList tl
...
export async function POST(request: NextRequest) {
  try {
    ...
    await pool
      ...
      .input('name', name ?? '')
      .query(`INSERT INTO TransplantList (id, projectId, name) VALUES (@id, @projectId, @name)`);
