...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    const listRes = await pool.request().input('id', id).query(
      `SELECT * FROM TransplantList WHERE id = @id`
    );
    if (!listRes.recordset.length) return notFound();
    const itemsRes = await pool.request().input('id', id).query(
      `SELECT * FROM TransplantListItem WHERE transplantListId = @id ORDER BY material ASC`
    );
...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    await pool
      ...
      .query(`UPDATE TransplantList SET name = @name WHERE id = @id`);
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    ...
    const pool = await getPool();
    const existing = await pool.request().input('id', id).query(
      `SELECT id FROM TransplantList WHERE id = @id`
    );
    if (!existing.recordset.length) return notFound();
    await pool.request().input('id', id).query(`DELETE FROM TransplantList WHERE id = @id`);
    return noContent();
  } catch (err) {
    return internalError(err);
  }
