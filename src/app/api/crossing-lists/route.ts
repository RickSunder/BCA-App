...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.request().input('pid', projectId).query(`
      SELECT cl.*,
        (SELECT id, female, male, plannedCount
         FROM CrossingListItem WHERE crossingListId = cl.id
         FOR JSON PATH) AS itemsJson
      FROM CrossingList cl WHERE cl.projectId = @pid ORDER BY cl.createdAt ASC
    `);
...
export async function POST(request: NextRequest) {
  try {
    ...
    await pool
      ...
      .input('name', name ?? '')
      .query(`INSERT INTO CrossingList (id, projectId, name) VALUES (@id, @projectId, @name)`);
