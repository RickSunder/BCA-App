...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.request().input('pid', projectId).query(`
      SELECT sl.*,
        (SELECT id, material, quantity, location
         FROM SowingListItem WHERE sowingListId = sl.id
...
export async function POST(request: NextRequest) {
  try {
    ...
    await pool
      ...
      .input('name', name ?? '')
      .query(`INSERT INTO SowingList (id, projectId, name) VALUES (@id, @projectId, @name)`);
